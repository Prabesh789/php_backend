...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($seller -> validate_params($_POST['email'])){
        ...
        echo json_encode(array('sucess'=> 0, 'message' => 'Email is required'));
        die();
    }
      ...
      if($seller -> validate_params($_POST['password'])){
        ...
        echo json_encode(array('sucess'=> 0, 'message' => 'Password is required'));
        die();
    }
    ...
    if($seller -> validate_params($_POST['address'])){
        ...
        echo json_encode(array('sucess'=> 0, 'message' => 'Address is required'));
        die();
    }
    ...
    if($seller -> validate_params($_POST['description'])){
        ...
        echo json_encode(array('sucess'=> 0, 'message' => 'Description is required'));
        die();
    }
    
    if($id = $seller-> register_seller()){
         echo json_encode(array('sucess'=> 1, 'message' => 'Seller registered!'));
    }else{
        http_response_code(500);
           echo json_encode(array('sucess'=> 0, 'message' => 'Internal server Error'));
    }
}else{
